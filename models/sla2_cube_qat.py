...
class SLA2CubeQATAttentionImpl(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        if needs_grad:
            ...
            # Linear-QK-Clip signals: max-minus-min is the true channel
            # softmax range. Max-minus-mean can be small while one channel
            # remains arbitrarily negative, so it cannot bound phi's minimum
            # mass or the denominator of the linear JVP quotient.
            with torch.no_grad():
                qf = qh.float()
                kf = kh.float()
                self.phi_range_q = (
