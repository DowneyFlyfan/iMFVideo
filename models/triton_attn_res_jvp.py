...
__all__ = ["triton_attn_res_jvp", "triton_attn_res_fwd"]


@triton.jit
def _attn_res_jvp_kernel(V, DV, W, Y, DY, J, D, eps,
...
def _launch(v, dv, w, eps):
    # v/dv: (n, J, d) contiguous;  w: (d,);  returns (y, dy) each (n, d)
    n, J, d = v.shape
    ...
    _attn_res_jvp_kernel[(n,)](
        v, dv if dv is not None else v, w, y, dy, J, d, eps,
        BLOCK=BLOCK, JP=triton.next_power_of_2(J),
        HAS_T=dv is not None,
        num_warps=4 if BLOCK <= 1024 else 8,
    )
    return y, dy


def triton_attn_res_jvp(v, dv, w, eps=1e-6):
